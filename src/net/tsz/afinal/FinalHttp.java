...
import net.tsz.afinal.utils.Common;
...
public class FinalHttp {
	 ...
	 * @param ctx
	 */
	public FinalHttp(Context ctx) {
		this();
		this.context = ctx;
		cookieStore = new PreferencesCookieStore(ctx);
		httpClient.setCookieStore(cookieStore);
		// configCookieStore(new PreferencesCookieStore(ctx));
	}
	...
	protected <T> void sendRequest(DefaultHttpClient client,
		...
		// 检测是否连接到网络
		if (context != null && !Common.isConnectingToInternet(context)) {
			ajaxCallBack.onFailure(new Exception("R.string.not_connected"), 0,
